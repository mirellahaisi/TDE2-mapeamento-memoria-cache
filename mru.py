...
def mru(sequencia, memoria_maxima, pagina_procurada):
    blocos = []        # quadros de memória
    tempo_uso = []     # ordem do uso (fim = mais recente)
    ...
    for pagina in sequencia:
        if pagina in blocos:
            ...
            if len(blocos) < memoria_maxima:
                ...
                # MRU substitui a mais recentemente usada (última da lista)
                mais_recente = tempo_uso.pop(-1)
                idx = blocos.index(mais_recente)
                blocos[idx] = pagina
                tempo_uso.append(pagina)
                print(f"Página {pagina}: substituiu {mais_recente}. Estado: {blocos}")
...
for t in testes:
    ...
    blocos, q = mru(t["sequencia"], memoria_maxima, t["alvo"])
    if t["alvo"] in blocos:
